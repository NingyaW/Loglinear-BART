...
double** Node::GetFits(void* model,int  nTrain,double** xTrain, double** xTrainR, double* yTrain, double* deTrain,double* wiTrain,double* vTrain,int nTest, double** xTest, double** xTestR, double* w)
{
	int i,j;
	...
	for(i=1;i<=nbot;i++)
	{
		...
		mod->setData(nobTrain,xTrainR,yTrain,deTrain,wiTrain,vTrain,indObsTrain,w);
...
void Node::currentFits(MuS* mod,int nTrain,double** xTrain,double* yTrain,double* deTrain,double* wiTrain,double* vTrain,int nTest,double** xTest,double* w, double **fits)
{
  double postmu,rhk,shk,W1,W2; //posterior of mu in a bottom node
  ...
  for(int i=1;i<=nbot;i++) { // loop over bottom nodes-------------
    ...
    //ybar = yTrain[itr[1]];
    //ch = wiTrain[itr[1]]*pow(exp(yTrain[itr[1]]),deTrain[itr[1]]);
    rhk = deTrain[itr[1]];
    shk = exp(yTrain[itr[1]])*vTrain[itr[1]];

    for(int j=2;j<=nobTrain;j++) {
      cell = cell->after;
      itr[j]=*((int *)(cell->contents));
      // ybar += yTrain[itr[j]];
     // ch *=wiTrain[itr[j]]*pow(exp(yTrain[itr[j]]),deTrain[itr[j]]);

     rhk += deTrain[itr[j]];
     shk += exp(yTrain[itr[j]])*vTrain[itr[j]];//yTrain[itr[j]];
   }
    ...
    for(int j=1;j<=nobTrain;j++) {
      if (std::isnan(rhk) || std::isnan(shk) || shk <= 0. || ( 2*shk == 0. && rhk-aa >= 0.)) {
        ...
        postmu=U*W1+(1-U)*W2;
        nodeMu = std::log(postmu);
        }

    // std::cout << "nodeMu=" << nodeMu << std::endl;
      fits[1][itr[j]] = nodeMu;
    }
...
double** Node::GetEstimates(void* model,int  nTrain,double** xTrain, double** xTrainR, double* yTrain,double* deTrain,double* wiTrain,double* vTrain, double* w)
{
	EndNodeModel* mod = (EndNodeModel*)model;
