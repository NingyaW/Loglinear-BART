...
double geGammaSample(double alpha, double beta) {
  return rgamma(alpha, 1.0/beta);
}

double computeZ(double n, double x, double psi) {
  if (n > 0 && x == 0 && psi > 0) {
    // Case 1: Gamma function with (2/psi)^n
    return gammafn(n) * pow(2.0 / psi, n);
  } else if (n < 0 && x > 0 && psi == 0) {
    // Case 2: Gamma function with (2/x)^-n
    return gammafn(-n) * pow(2.0 / x, -n);
  } else if (x > 0 && psi > 0) {
    // Case 3: Modified Bessel function of the second kind
    double kn = bessel_k(sqrt(psi * x), n, 1);
    return 2 * kn / pow(psi / x, n / 2.0);
  } else {
    // Handle other cases or invalid inputs
    throw std::invalid_argument("Invalid arguments for function computeZ.");
  }
}
